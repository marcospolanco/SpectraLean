...
| 3 | Variational and functional methods | 4.5 | `quadForm`/Dirichlet identity and PSD **proved**; `rayleigh` defined; normalized↔combinatorial transfer **proved** (2026-08-17, `VariationalTransfer`: congruence lemma, degree-weighted Rayleigh quotient `rayleigh L_sym (√D y) = (yᵀ L y)/∑ deg·y²`, `normalizedLaplacian_psd`); **λ₂ variational characterization (Courant–Fischer) now proved** (2026-08-18, `lambda2_variational`: `λ₂ = sInf` of the Rayleigh quotient over nonzero vectors ⊥ `onesVec`, for symmetric nonnegative weights — retired from axiom, whose symmetry-only shape was false and is refuted in QA; matrix-world proof through eigenbasis expansion, Parseval, the spectral resolution of `quadForm`, and two sorted-multiset multiplicity pins; the semidefinite Cauchy–Schwarz `laplacian_cauchy_schwarz` is also proved); **general Courant–Fischer min–max proved at every index** (2026-08-18, proposal `prove-courant-fischer.md`: `evals_min_max` with the two witness directions `exists_submodule_forall_rayleigh_le` / `exists_ne_mem_rayleigh_ge_of_finrank_eq` — only symmetry assumed, no positivity or graph structure; the general-`k` multiplicity pins and component-form Rayleigh bounds are new public center API; QA pins both directions on a spectrum-`[1,3]` fixture, derives the top-eigenvalue domination universal through the theorem, and refutes wrong and under-dimensional subspaces plus an interior-index wrong two-dimensional subspace on the path Laplacian). Re-scored 3.5 → 4.0 with this milestone (the fixed-index engine generalized to the full subspace min–max; the axis's remaining named gaps are functional inequalities). **The Poincaré inequality family is delivered** (2026-08-31,
`proposals/poincare-inequality.md`, the new
`GraphTheory.Poincare`, zero new axioms): the combinatorial form
`∑ (f − mean f)² ≤ fᵀLf/λ₂` (the division-free engine carrying no
λ₂-positivity hypothesis at all), the degree-weighted π form
`∑ deg·(f − E_π f)² ≤ fᵀLf/λ₂(L_sym)` through the general-kernel
Rayleigh bound at `√D(f − E_π f·1)` and the congruence `√D L_sym √D = L`
(with connected twins), and the **spectral edge-expansion corollary**
`λ₂·|S|·(|V|−|S|)/|V| ≤ boundary A S` — linear in the gap, set-by-set,
consuming `Multiway.lean`'s indicator energy identity (its first
consumer outside its own module). QA: exact attainment at Fiedler
vectors on K₂/P₃/K₃ (2 = 2, 2 = 2, 4 = 4), the wrong-constant
refutation at the K₂ fixture, and the no-constant connectivity fence
(on the disconnected two-edge fixture, variance 1 with energy 0 — no
finite constant works, the `0 < λ₂` guard exactly the true region's
boundary). Re-scored 4.0 → 4.5: the axis's charter names Poincaré as
its functional-inequality pillar beside the variational methods, and
this is a proved, composable family with a new cut-level consumer —
not packaging. **The connectivity transfer is proved** (2026-08-26, the irregular family's own priced follow-on, pure hard crust): `secondEval_normalizedLaplacian_pos_iff_connected` — `0 < λ₂(L_sym) ↔ connected` on every symmetric nonnegative positive-degree graph — through the kernel characterization `normalizedLaplacian_mulVec_eq_zero_iff` (the stretched-constant line `√D·1`, via the electrical program's combinatorial kernel characterization), the Fiedler mirror `secondEval_normalizedLaplacian_pos_of_connected` (PSD + sorted + multiplicity pin + orthonormality), the disconnected converse via the component-indicator Gram–Schmidt route through `secondEval_le_rayleigh_of_ker`, and the Cheeger consumer corollary `cheegerConstant_pos_of_connected` (`0 < φ` on connected irregular graphs, joined to the delivered easy direction). Absent: log-Sobolev (Poincaré delivered 2026-08-31; **the recorded consumer gate — "a named consumer for an entropy inequality composing `InformationTheory.Entropy`'s Gibbs bound" — was discharged 2026-09-01 by the entropy-mixing delivery** (`proposals/entropy-mixing-pinsker.md`: the entropy-decay family is on the shelf, its two-block log-sum engine literally composing `sub_le_klTerm`, and an LSI would improve the decay bound at its own rate — adopting LSI is now an operator decision with a concrete consumer, not a missing-machinery gap). |
...
| 5 | Random walks and diffusion | 4.5 | Regular and irregular transition matrices with row-stochasticity **proved**; walk↔normalized similarity and both Laplacian bridges **proved**; the walk-matrix **eigenpair transfer proved** (2026-08-22, `proposals/mixing-time-bound.md` Step 1: eigenpairs conjugate through the similarity in both directions, `walkEvals` certifying each transferred entry a genuine eigenvalue of the non-symmetric walk matrix with explicit nonzero eigenvector witnesses, and `walk_eigvec_expansion` the diagonalizability interface — the axis's named walk-spectrum gap, closed as QA'd hard crust; re-scored 2.5 → 3.0); **reversibility (detailed balance) proved** (2026-08-22, `proposals/reversibility-and-heat-semigroup.md` Phase A: `deg i * P i j = deg j * P j i` with both forms — degree measure and stationary measure `π = deg/vol` — plus the symmetrizability packaging `D * P` symmetric and the regular uniform-measure corollary, zero new axioms, in `GraphTheory.Stationary`); **the ℓ²-mixing proxy defined with its evolution interface** (2026-08-22, `proposals/mixing-time-bound.md` Step 2, the new `GraphTheory.Mixing`: `stationaryVec`/`walkDistribution`/`walkDensity`/`chiSquareDistance` all proved-interface hard crust, `walkDensity_succ` consuming the detailed-balance interface — the density-coordinate evolution `h_{t+1} = P *ᵥ h_t` that the transferred eigenbasis diagonalizes); **the geometric decay engine proved** (2026-08-22, Step 3 component 1: the conjugated-power transfer `√D *ᵥ (Pᵗ *ᵥ g) = (1 − L_sym)ᵗ *ᵥ (√D *ᵥ g)`, eigencoordinate evolution, the Parseval-exact decay identity, and the ℓ²(π) contraction under value-based mode exclusion); and **the χ² mixing statement itself proved — the program's closing theorem** (2026-08-22, Step 3 component 2: `chiSquareDistance_le_of_connected` — `χ²(t, x) ≤ r ^ (2t) · ((π x)⁻¹ − 1)` on connected symmetric-nonnegative positive-degree networks under the rate hypothesis — assembled from the centered density evolution `h_t − 1 = Pᵗ *ᵥ (h₀ − 1)`, the connectivity kernel characterization of `L_sym` (the shelf's `laplacian_kernel_eq_span_onesVec` transferred through the proved congruence `√D L_sym √D = L`, composed with mass conservation), and the delivered contraction; zero new axioms; QA pins the bound **attained exactly** on K₃ at `t = 1, 2` (`1/2 ≤ (1/4)·2`, `1/8 ≤ (1/16)·2` — equalities), the λ* = 1 no-decay behavior on P₃ (rate derived basis-independently from two sum-of-squares certificates pinning the spectrum into `[0, 2]`), and refutes the connectivity-dropped form on a triangle⊕self-loop fixture where the rate hypothesis provably holds at `r = 1/2` yet `χ²(3) = 3/8 > 3/64` — the loop's `μ = 0` mode being exactly the hole connectivity plugs; re-scored 3.0 → 3.5). Heat semigroup **in delivery** (2026-08-23, `proposals/reversibility-and-heat-semigroup.md` Phase B Steps 0–3, the Active table's single High row — its gate resolved by the real external consumer `sgt-gaps.md`: the new `GraphTheory.Heat` defines the matrix-level `heatKernel A t = e^{-tL}` on `laplacian A` with symmetry under `A.IsSymm` through the pin's `Matrix.IsSymm.exp`, the hypothesis-free time-zero identity, the square-zero exponential collapse, **the semigroup law** `heatKernel A s * heatKernel A t = heatKernel A (s + t)` (Step 2, delivered 2026-08-23: hypothesis-free, via `Matrix.exp_add_of_commute` at the commuting negated scalar multiples, QA witnessing the law by two independent routes — hand multiplication of the fixture's closed forms vs. the theorem), and **mass conservation** `heatKernel A t *ᵥ onesVec = onesVec` (Step 3, delivered 2026-08-23: hypothesis-free, through the entrywise-built exponential-series convergence `expSeries_hasSum_exp` — the summability content constructed from scratch because the pin's normed `expSeries_summable'` cannot be applied at matrix type and the pin has no Pi-HasSum lemmas at all — and the kernel-vector engine `exp_mulVec_eq_of_mulVec_eq_zero`, whose per-kernel-vector generality the QA consumes for the per-component no-leakage witness on a disconnected fixture; the raw-vs-theorem two-route conservation check and the non-kernel-vector guard complete the witness set) — zero new axioms; held at 3.5 pending Step 4); and **the heat-kernel *statements* themselves — eigenmode decay and the DC limit — proved, closing the heat family** (Step 4, delivered 2026-08-23: the eigenmode engine `exp_mulVec_eq_smul_of_mulVec_eq_smul` (`M *ᵥ v = μ • v → exp ℝ M *ᵥ v = e^μ • v`, Step 3's convergence consumed at an eigenvector, the kernel engine its `μ = 0` case), `heatKernel_mulVec_eigvecOf` (every eigenbasis vector an eigenvector of `heatKernel A t` at every time, eigenvalue `e^{−t·λᵢ}`), the sorted-spectrum decay monotonicity with the PSD `≤ 1` dissipation bound, the eigenbasis expansion `heatKernel_mulVec_eq_sum` (the spectral-calculus identity in action form), and the payoff **DC limit** `heatKernel_mulVec_tendsto_atTop` (on connected graphs the heat flow of any vector converges to its mean `((∑ j, x j)/|V|) • onesVec` — assembled from PSD, `det L = 0` + `det_eq_prod_eigenvalues`, the kernel characterization, uniqueness-by-orthogonality, and finite-sum limit passage; zero new axioms; QA: two independent routes to the mode-decay statement and to the DC limit on K₂, the new rank-one-idempotent closed form `heatKernel K₂ t = 1 + ((e^{−2t}−1)/2) • L`, the K₂ spectrum pinned `[0,2]` from trace/determinant, and the engine⇄collapse cross-validation at a nonzero eigenvalue; re-scored 3.5 → 4.0 — the named trigger fired, and the continuous-time side of the axis is now a closed capability family: definition, identity, semigroup, conservation, mode decay, convergence). **The ℓ² → total-variation conversion delivered 2026-08-31** (`proposals/total-variation-mixing-conversion.md`, the mixing program's deferred Step 4 as its own same-run proposal — the axis's last named absent category, closed; re-scored 4.0 → 4.5): `tvDistance` in its finite-state vector form with the generic Cauchy–Schwarz conversion `tvDistance_le_half_sqrt` at the sharp classical constant `TV ≤ (1/2)·√χ²` (sign-free on `ν`, hence strictly stronger than the textbook probability-vector form; the constant **attained exactly on `K₂` at `t = 1`** — `k2_conversion_attained_QA`, the strongest QA shape a bound theorem can have), the unconditional walk-level shadow, the rate form at exactly the χ² theorem's hypothesis set, and — in `Oversmoothing.lean`'s TV-twin section — the depth-form TV ceiling (the field-standard `t_mix(ε)` statement: past the entrywise ceiling's own threshold at `2ε`, the walk law is within `ε` of stationarity *in total variation*) with the two-start `2ε` twin; zero new axioms, the mass-one hypothesis fenced in QA. The axis's named remaining gaps are now only forward extensions (a defined `t_mix` object — consumer-gated; the reverse TV → χ² calculus — no consumer named). **The continuous-time χ² mixing bound was delivered 2026-08-31** (`proposals/continuous-time-chi-square-mixing.md`, same-run proposal and delivery — the heat-variance delivery's named π-weighted twin with the consumer gate discharged by naming the field-standard continuous-time mixing statement; zero new axioms; score **held at 4.5** per protocol: a consumer composition within the already-counted mixing + heat families, not a new theorem family): the walk heat kernel `e^{-tL_walk}` with the `√D`-conjugation to `e^{-tL_sym}` (the continuous analogue of the shelf's conjugated-power transfer), the π-weighted variance decay `∑ deg·(e^{-tL_walk}f − mean_π f)² ≤ e^{−2tλ₂(L_sym)}·∑ deg·(f − mean_π f)²` (hypothesis-minimal), and the consumer `contChiSquareDistance_le` — `χ²_cont(t,x) ≤ e^{−2tλ₂(L_sym)}·((πx)⁻¹−1)` at the discrete theorem's hypothesis set *minus connectivity* and with **no caller-certified rate** (the continuous rate is intrinsic, where the discrete theorem needs an `r` certificate — a strictly stronger statement shape than its discrete sibling); QA pins attainment at every time on the triangle, the disconnected exact form `1/3 + (8/3)e^{−3t}`, and fences on both fixtures. **The continuous-time mixing time was delivered 2026-08-31** (`contMixingTimeFrom` — the per-start `t_mix(ε)` at exactly LPW ch. 20's `∀ s ≥ t` reading, the repo's first defined `sInf` time object on the mixing axis; with the continuous-time ℓ²→TV conversion and decay form, the certificate interface, the spectral ceiling `t_mix(ε) ≤ max 0 (ln(√((πx)⁻¹−1)/(2ε))/λ₂(L_sym))`, and ε-antitonicity; QA +26 including the exact `K₂` closed form `t_mix(ε) = ln(1/(2ε))/2` pinned in both directions; score **held at 4.5** per protocol: consumer composition within the already-counted TV + continuous-χ² families). **The Poisson bridge — the continuous↔discrete comparability — was delivered 2026-09-01** (`proposals/continuous-time-chi-square-mixing.md`'s named follow-on, run `20260901T024900Z-run-1` with the records completed by `20260901T054637Z-run-1`; zero new axioms, pure hard crust — all 42 audited declarations at the standard three; QA +19, 3459 → 3478): **the Poissonization identity** `ν^cont_t = ∑'ₖ e^{−t}tᵏ/k! · ν_k` (the mixing layer's first tsum construction, through `Matrix.exp_add_of_commute` at `−t(I−P) = tP − tI` plus the scalar-matrix exponential), the **TV contraction toolkit** (the `Pᵀ` ℓ¹-contraction and **discrete TV monotonicity** — field-standard facts new to the shelf), mixture convexity, **the comparability** `TV_cont(t) ≤ ∑_{k<m} e^{−t}tᵏ/k! + TV_disc(m)` with the tail exact, and **the discrete-certificate transfer corollary** whose `hmix` clause is exactly the still-deferred discrete `t_mix` object's witness condition — that object's consumer now exists on the shelf. QA carries the recorded hazard as a witness: on the periodic chain `K₂` the discrete TV is `1/2` at every time (no discrete certificate exists; the **reverse comparability is dead** — `TV_cont(2) = e^{−4}/2 < 1/4` against `TV_disc ≡ 1/2`), the `t = 0` corner collapses to `δ_x`, and the triangle closes in exact closed forms (`TV_disc(m) = (2/3)·2^{−m}`, the transfer instance `TV_cont(8) ≤ 5/24`). Score **held at 4.5** per protocol: the comparability is the join of the already-counted TV and continuous-time families through a new tsum construction, not a new theorem family. **The discrete `t_mix` object was delivered 2026-09-01** (`proposals/total-variation-mixing-conversion.md`'s deferred object as a follow-on record, run `20260901T070242Z-run-1`; zero new axioms — all 16 audited declarations at the standard three; QA +9, 3478 → 3487): `walkMixingTimeFrom` (the per-start `sInf` over witness times, LPW ch. 20's reading — the discrete twin of `contMixingTimeFrom`) with the certificate interface, **the attainment specification** (a *minimum*: `ℕ` well-ordered, `csInf_mem`, membership itself the uniform bound — the fact that discharges the Poisson-bridge transfer corollary's `hmix` clause, closing the consumer loop in the direction the bridge opened), **the spectral ceiling** `t_mix(ε) ≤ ⌈log(√C/(2ε))/log(1/r)⌉` at the depth-form TV certificate's own `0 < r < 1` hypothesis set (honest strictness — periodic chains admit no such certificate, the `K₂` junk-corner fence), ε-antitonicity, and **the bridge composition** (the named consumer: continuous TV ≤ ε₁ + ε₂ with the discrete certificate supplied by the object). Score **held at 4.5** per protocol: the packaging of already-counted TV-certified depth families into their field-standard object, not a new theorem family. The axis's forward extensions are now the reverse TV → χ² calculus (no consumer named) and log-Sobolev (axis 3's gate) — the submultiplicivity class and the sup-over-starts uniform `t_mix`, both previously listed here, are delivered below. **The spectral mixing floor was delivered 2026-09-01** (`proposals/message-passing-depth-mixing-bound.md`'s deferred over-squashing item on its own named route, run `20260901T091923Z-run-1`; zero new axioms — all 39 audited declarations at the standard three; QA +30, 3487 → 3517): the program's *first lower-bound family* — the distinguishing-function bound `tvDistance_ge_half_abs_sum` (hypothesis-minimal), the exact law-level test-function evolution at arbitrary `L_sym` eigenpairs (the deferred item's engine: an equality, not a bound), the TV floor `(1/2)|1−μ|^t·|v x|/(√D x·c) ≤ TV` and the χ² floor `(1−μ)^{2t}(v x)²/(π x·‖v‖²) ≤ χ²` (kernel-orthogonality *derived* from `μ ≠ 0`, no connectivity; periodic `|1−μ| = 1` modes never decay — exactly the chains no `r < 1` ceiling reaches, with QA attaining the floors exactly on `K₂` at every time), the `t_mix` floor gate (witness-existence load-bearing, fenced), and the log-form floor `⌈log(|v x|/(√D x·2εc))/log(1/|1−μ|)⌉ ≤ t_mix` — the classical eigenvalue lower bound on mixing time, hitting all three pinned triangle `t_mix` closed forms exactly and completing the two-sided depth bracket (floor `2` = truth `2` ≤ ceiling `3` at `ε = 1/6`). Score **held at 4.5** per protocol: a direction-reversed consumer of the already-counted eigenbasis and TV families, not a new theorem family. **The uniform mixing time with the submultiplicativity class was delivered 2026-09-01** (`proposals/total-variation-mixing-conversion.md`'s deferred uniform-`t_mix` family as a follow-on; spike by run `20260901T124410Z-run-1`, landing by run `20260901T163200Z-run-1`; zero new axioms — all 46 nameable audited declarations at the standard three via `wip/uniformmix_axcheck.lean`; QA +27, 3517 → 3544): LPW's two distances `walkTVPair`/`walkTVUniform` (`d(t)` = the worst pair, `d̄(t)` = the worst start — Montenegro–Tetali's), **the sharp Dobrushin contraction** `TV(μ(Pᵀ)ᵗ, ν(Pᵀ)ᵗ) ≤ TV(μ,ν)·d(t)` at equal masses (hypothesis-minimal — no stochasticity: the sign statistic of the evolved difference paired through the recentering-pairing core against the delivered distinguishing-function bound, avoiding the factor-`2`-losing triangle route), the submultiplicativity class `d(s+t) ≤ d(s)d(t)` (pure Markovity), `d̄ ≤ d`, `d̄(s+t) ≤ d̄(s)d(t)`, and the escalation engine; **the uniform object** `walkMixingTime` (LPW ch. 20's field-standard worst-case-start `t_mix(ε)`, on a finite type the worst start's per-start object — the sup-inf interchange) with the certificate/attainment interface and the witness-hypothesized per-start domination (the witness load-bearing, fenced at the `K₂` junk corner); **the ε-escalation corollary** (one certified evaluation time yields every ε-level mixing time — LPW's `t_mix := t_mix(1/4)` bridge, both `k`-form and ⌈log⌉ display) and the uniform spectral ceiling. QA: submultiplicativity **attained with equality** on the triangle in all three forms (the sharp Dobrushin constant load-bearing), the uniform object's exact closed forms `t_mix(1/3) = 1`, `t_mix(1/6) = 2` pinned in both directions, and the `K₂` periodicity corner (`d(1) = 1` maximal, no `ρ < 1` certificate exists, the empty-set infimum pinned and fenced). Score **held at 4.5** per protocol: the field-standard packaging of the already-counted TV families into LPW's canonical object, not a new theorem family. **The lazy mixing time was delivered 2026-09-01** (`proposals/lazy-mixing-time-objects.md`, the lazy-walk delivery's two recorded follow-on compositions with the consumer gate discharged by naming the bipartite-input instance — the empirical-stationary capstone's own "agent that can only simulate the walk" setting on paths/trees/grids, where the plain `r < 1` certificate is provably unsatisfiable; zero new axioms — all 20 audited declarations at the standard three; QA +11, 3589 → 3600): the `t_mix(ε)` object at the lazy law (`lazyWalkMixingTimeFrom`) with the certificate interface, the inherited `csInf_mem` attainment, ε-antitonicity, and **the intrinsic-rate spectral ceiling** `t_mix_lazy(ε) ≤ ⌈log(√((πx)⁻¹−1)/(2ε))/log(1/(1−λ₂/2))⌉` (the honest visible `λ₂ < 2` — K₂'s rate-0 corner excluded, its object pinned exactly instead), the depth-form TV lazy ceiling, and **the entrywise lazy ceiling at the intrinsic rate** (connectivity the only graph hypothesis — the plain twin's certificate hypothesis replaced by the computed rate, on the class where the plain twin is provably unusable), plus the spectrum cap `λ₂(L_sym) ≤ 2` in `Mixing.lean`. QA: the exact TV closed form `TV_lazy(1+t) = (1/2)^{t+2}` on the bipartite path's corner start, `t_mix_lazy` closed forms pinned in both directions on `P₃` and `K₂`, the ceiling attained exactly (center start) and with honest slack (`3` vs `2`, corner start), and **the object-level periodicity contrast** — plain `t_mix(K₂, 1/8) = 0` junk against lazy `= 1` genuine. Score **held at 4.5** per protocol: the packaging of the already-counted lazy rate family into its field-standard object plus the cap, not a new theorem family. Sixth extension (2026-09-02, `proposals/directed-uniform-mixing-time.md`): **the `t_mix` object family is now complete** — the directed uniform (worst-start) sibling `pageRankMixingTime` delivered with LPW's `d`/`d̄` at the Google law, the submultiplicativity class and ε-escalation corollaries through the mixing layer's new matrix-level Dobrushin-coefficient engine (`tvDobrushinCoeff` + the sharp contraction `TV(μ ᵥ* Q, ν ᵥ* Q) ≤ TV(μ,ν)·δ(Q)` + generic power submultiplicativity — the undirected uniform delivery's mechanism promoted to any row action), the refined α-ceiling at `d̄(0)`, and the worst-start sampling capstone (one start-independent threshold for every start); score **held at 4.5** per protocol (the field-standard packaging of the already-counted directed rate family into its worst-start object plus a reusable engine promotion, not a new theorem family). |
...
**Weakest axes:** 8 (adjacent systems — deliberately gated) and, at
4.0–4.5, the middle ring. Axis 5 (random walks) sits at 4.5: the full
mixing-time program (eigenpair transfer, ℓ²-mixing proxy, geometric
decay engine, the closing χ² mixing bound), the complete
continuous-time heat-kernel family (`e^{-tL}` with identity, semigroup
law, mass conservation, eigenmode decay, and the connected-graph DC
limit — the reversibility proposal's Phase B, closed 2026-08-23 as
hard crust with two-route QA), the oversmoothing ceiling with its
per-pair resistance refinement, and — closing the axis's last named
absent category, 2026-08-31 — the ℓ² → TV mixing conversion
(`proposals/total-variation-mixing-conversion.md`, the field-standard
`t_mix(ε)` statement form on the shelf), and — 2026-09-01 — the
entropy leg (`proposals/entropy-mixing-pinsker.md`: Pinsker in vector
form, the entropy–χ² bridge, entropy decay with the continuous-time
twin, and the entropy floor — `InformationTheory.Entropy`'s first
graph-level consumers; held at 4.5 per protocol as a third distance
joined to the already-counted mixing families) — and the same day the
**lazy walk** (`proposals/lazy-walk-mixing.md`: `P_L = (P+I)/2` with
the intrinsic-rate χ²/TV/entropy family and the signless SOS
`μ(L_sym) ≤ 2` — LPW ch. 5's periodicity fix making the discrete
certificate families satisfiable on bipartite graphs; held at 4.5 per
protocol as the already-counted families' lazy time-change with one
new elementary engine). Axis 3 (variational and
functional methods) also sits at 4.5 since the Poincaré delivery
(2026-08-31, `proposals/poincare-inequality.md`); its remaining named
gap is log-Sobolev.
...
| Axiom minimization | 4.5 | 4 explicit axioms, all cited and indexed (**also synced 2026-09-02 with the Cesàro stationary re-proof, `proposals/cesaro-stationary-existence.md`: `perron_frobenius`'s entire theorem-consumer surface — `IrreducibleStationary`'s engine + four stationary theorems, `PageRank`'s three, every `DirectedMixing`/empirical instantiation producing `π` — re-proved without the axiom at unchanged statements, `#print axioms` exactly the standard three across 37 audited declarations; the axiom stays admitted at its own full statement but now has zero non-QA consumers, the deprecation decision flagged as the operator's**; earlier: synced 2026-09-02 with the `primitive_power_tendsto` retirement — 5 → 4, `proposals/retire-primitive-power-convergence.md`: the admission's own "Replacement path" note had left the local route unpriced; the Doeblin/Dobrushin entrywise-range contraction proves the statement unchanged with no Perron–Frobenius machinery, the contraction landing as a reusable public engine (`entryRange_mulVec_le_of_pos_entries`), and the entire directed mixing layer — the PageRank power iteration plus both generic corollaries — turning hard crust at `#print axioms` standard-three, 41 audited declarations; the remaining four are `perron_frobenius`, `matrix_hoeffding`, `matrix_bernstein`, `matrix_azuma_hoeffding`; score held at 4.5 per protocol — a retirement within the already-cited-and-QA'd admitted set, the same trend-continuation class as the 2026-08-30 retirements) (earlier: synced 2026-08-30 with the `hoeffding_lemma` pointwise-collapse retirement — **`proposals/retire-hoeffding-lemma-pointwise-collapse.md`: the ψ₂-form statement proved at its unchanged shape through the sharper companion `subgaussianNorm_le_of_bounded` (`≤ a/√(log 2)`, no centering, no pins, constant attained), 6 → 5 — the last scalar concentration axiom and the last zero-consumer axiom both eliminated, every remaining admitted axiom now consumer-covered**; the row-head count had drifted stale at 10 across the two earlier 2026-08-30 retirements — repaired with that sync, the same drift class the 2026-08-24 sync fixed; **the Hoeffding retirement 2026-08-30** (`proposals/prove-hoeffding-inequality-mgf.md`: `hoeffding_lemma_mgf` + the Chernoff assembly prove `hoeffding_inequality` and `hoeffding_empirical` at unchanged statements, 10 → 8, the largest single trust-surface reduction by axiom count in the project's history) **and the Bernstein repair-and-retirement 2026-08-30** (Errata §8, `proposals/repair-and-retire-bernstein-pair.md`: both axioms materially false in hypothesis shape — the *uncentered* bound where the sources bound the centered variables — repaired to the source-faithful centered shape and proved by the local Bennett MGF engine, 8 → 6, the Bernstein-twin Derived tails becoming hard crust with them); score held at 4.5 per protocol — retirements within the already-cited-and-QA'd admitted set, the falseness class again repaired in the open; the count reached 10 on 2026-08-20 with the Weyl retirement — `weyl_inequality` proved at its unchanged statement from the Courant–Fischer engine plus the operator-norm bridge, `proposals/discharge-perturbation-axioms.md`; the perturbation bridge's admitted set is now Davis–Kahan alone, and the derived projector-drift chain is conditional on two axioms instead of three); `spectral_gap_stability`, `hoeffding_iid`, `bernstein_iid` converted from admitted to proved; **`lambda2_variational` retired from axiom to proved theorem (2026-08-18) — the first axiom removed by proof rather than deprecation — and `cheeger_upper_bound` retired the same way one milestone later** (the Cheeger easy direction, proved from the generalized `secondEval_variational`; both false pre-repair shapes refuted in QA); **`woodbury_identity` retired 2026-08-18 as an emergency correctness repair — the first retirement motivated by the axiom being verified *false* rather than merely unproved, proved from Mathlib's `Matrix.invOf_add_mul_mul` at zero local proof cost**; **`sherman_morrison` retired 2026-08-18 as the `k = Fin 1` specialization of the proved Woodbury theorem — a pure proof task at unchanged name/hypotheses (the statement had been verified correct against the repaired Woodbury shape first), completing the matrix-update bridge at zero axiom cost**; **`eigen_interlacing_principal_submatrix` retired 2026-08-18 — Cauchy interlacing proved from the locally proved Courant–Fischer engine, the engine's first named consumer and the first SGT-center retirement (all prior retirees were variational, update-identity, or derived statements)**; `spectral_persistence` deprecated 2026-08-17 with a migration note and removed 2026-08-20 (zero non-QA consumers throughout; the derived two-endpoint chain covers the motivating use). Net trend 26 → 19 → 18 → 17 → 16 → 15 → 14 → 13 → 12 → 11 → 10 → 9 → 10 → 9 → 10 → 8 → 6 (the 2026-08-24 up-step is the `primitive_power_tendsto` admission, Horn & Johnson §8.5 at the row-stochastic specialization — the directed axis' first *convergence* axiom, the standing no-dominance scope of `perron_frobenius` being precisely why it was needed, fenced by the 2-cycle periodicity refutation; the 2026-08-25 Hoffman delivery touches no axiom — pure hard crust, the count held);the 2026-08-21 step is `davis_kahan_sin_theta`, proved by the Duhamel route of `proposals/discharge-perturbation-axioms.md`; the 2026-08-22 down-step is `subgaussian_tail_bound`, the **second false-axiom repair retirement** after Woodbury — the mandated Step 0 spike proved the old `subgaussianNorm ≤ K`-shaped axiom materially false through two independent junk mechanisms (`Real.sInf_empty` vacuity; `integral_undef` junk-zero MGF integrals making the defining set full for heavy tails), refuted in QA with the old hypotheses satisfied, and the theorem restated at the same name/conclusion with the moment integrable — the falseness category is empty again among checked statements, and the first measure-theoretic retirement landed; the 2026-08-22 up-step is the Perron–Frobenius admission (the directed axis' second spectral toolkit — a deliberate admission, not drift); the 2026-08-23 down-step is `cheeger_lower_bound`, the Cheeger hard direction proved at the unchanged statement by the median-split route of `proposals/discharge-perturbation-axioms.md` — the discharge program complete: Weyl, Davis–Kahan, and both Cheeger directions all proved; the count also passed through 11 on 2026-08-20 with the operator-directed `spectral_persistence` removal, an editorial deletion rather than a proof retirement). Score held at 4.5 (a retirement within the already-cited-and-QA'd admitted set; the falseness category stays empty). Score 4.5 (re-scored 4.0 → 4.5 at the Woodbury repair: the false-axiom category is now empty among statements whose truth has been checked, and the false Woodbury shape was removed rather than merely replaced; the Sherman–Morrison and interlacing retirements continue the trend without crossing a new threshold — the Core update-identity bridge and the interlacing window are now axiom-free; the Weyl retirement likewise continues the trend — the first *perturbation-bridge* retirement, but within the same cited-and-QA'd admitted set rather than a new threshold; the subgaussian repair holds the score — the second false-axiom repair confirms the falsification discipline works, but the discovered falseness itself is not minimization progress to score). |
...
| QA | 4.5 | 4012 declarations across 68 modules (count synced 2026-09-03 with the entropy-family fence QA +130, 3882 → 4012 — `Entropy_QA.lean`'s Section F + `Mixing_QA.lean`'s `EntropyFences` section, `proposals/adversarial-fences-entropy-family.md`: **negative witnesses for the entropy family's 28 unfenced load-bearing clauses**, the third mixing-cascade family audited and the only one whose QA (2026-09-01) predates the adversarial-review discipline — the mass clauses of the entire generic KL/entropy layer (`klDiv_nonneg`, `klDiv_eq_zero_iff` in both its strict-positivity and mass readings, `klDiv_apply_uniform`, all three `shannonEntropy` statements incl. the transcendental `H(1/3,…) = (4/3)·log 3 > log 4` fence by `3⁴ > 4³`, the termwise bridge's `b = 0` junk corner, the entropy–χ² bridge's `p`-mass clause at the `(3/4,3/4)` fixture where `D ≥ 1/2 > 1/4 = χ²` — the sum-level conversion genuinely load-bearing — the two-block log-sum bound's `q`-zero corner, and both strict-positivity clauses of the binary two-point engine); both mass clauses of Pinsker's inequality (a junk-negative divergence makes the square root `0` against `TV = 1/4` and `1/2`); the decay form's `hrate` at the triangle's failing `r = 1/8` certificate; `hnn`/`ht` of the continuous nonneg plumbing through closed-form heat kernels (`exp_eq_one_add_of_mul_self_eq_smul`) at the negative off-diagonal fixture and the `K₂` backward semigroup `t = −1`; `hA`/`hd` of continuous mass conservation (a new asymmetric-swap fixture with an idempotent walk Laplacian — mass `3 − 2e^{−1} ≠ 1` — and the `zdAdj` junk-zero law against the `0⁻¹ − 1 = −1` bound, no eigenvalue pin needed by `exp_pos`); and all four analytic clauses of the entropy floor (`hμ` at the genuine kernel eigenpair, `hv` at a fake `μ = 3` mode, `hc` at an undershooting sup bound, `hA` at the swap's genuine `(1, ![0,1])` eigenpair of its asymmetric `L_sym`) — each with isolation companions, plus recorded non-fenceables (junk log-of-negative contamination on every `p`-side clause, vacuous empty-type corners, the proof-shaped `hconn`, and three removable-hypothesis findings, notably the continuous nonneg `hA` clauses: the heat kernel `e^{−t}·e^{tP}` is entrywise nonnegative for *any* nonnegative `P`, symmetric or not — the delivered proofs use `hA` via Poissonization, the truth does not); score held at 4.5 per protocol — negative witnesses of an already-counted family, not a named-gap closure; the *randomized* half of the axis's named gap remains absent). Earlier sync: 3882 declarations across 68 modules (count synced 2026-09-02 with the lazy-family fence QA +65, 3817 → 3882 — `Mixing_QA.lean`'s `LazyFences` section, `proposals/adversarial-fences-lazy-family.md`: **negative witnesses for the ten load-bearing hypotheses of the lazy family the 2026-09-01 delivery left unfenced** — the `hd` clauses of lazy row-stochasticity, mass conservation, and the `t = 0` χ² normalization (refuted at a zero-degree fixture where the `D⁻¹A` row is junk-zero: `1/2 ≠ 1`, mass `1/2 ≠ 1`, normalization `1 ≠ 0⁻¹ − 1 = −1`), the `hnn` clauses of operator- and law-nonnegativity (refuted at a negative *off*-diagonal fixture: `P_L 0 1 = −1/2` — the existing negative-diagonal fixture cannot kill them, its off-diagonal `2` keeping every `P_L` entry nonnegative), the `hA` clauses of detailed balance (`3/16 ≠ 1/16`), stationarity (`5/8 ≠ 3/4`), and attainment persistence (the asymmetric fixture's lazy law hits `π` exactly at `t = 1` and leaves it at `t = 2` — the hypothesis genuine, the conclusion refuted), and both certificate clauses of the public lazy ℓ²(π) contraction engine (`hrate` at the triangle's genuine `3/2`-mode direction with `r = 1/8` below the factor `1/4`: `1/24 > 1/96`; `hmode` at the edge's constant mode with the genuine `r = 0`: `1 > 0`) — each with an isolation companion proving the refuted clause is exactly what fails at the fixture (the edge's spectrum pinned by the new `k2_eigvalOf_cases_QA` with no basis control); recorded non-fenceable with reasons: the headline's `hconn` (the dropped statement stays *true* on disconnected input — proof-shaped, not truth-shaped) and the depth ceiling's `hslt` (junk-prevention); a removable-hypothesis finding recorded for the two nonnegativity `hd` clauses; score held at 4.5 per protocol — negative witnesses and isolation companions of an already-counted family, not a new QA capability family; the *randomized* half of the axis's named gap remains absent). Earlier sync: 3817 declarations across 68 modules (count synced 2026-09-02 with the adversarial-fence QA +49, 3768 → 3817 — `DirectedMixing_QA.lean`'s Section I, `proposals/adversarial-fences-tv-dobrushin-engines.md`: **negative witnesses for the seven load-bearing hypotheses of the TV/Dobrushin engine family the 2026-09-02 directed-rate deliveries left unfenced** — the equal-mass clauses of both TV contractions (refuted at the all-half matrix whose Dobrushin coefficient is exactly `0`: the mass-dropped contraction reads `1/2 ≤ 0`), the row-sum and nonnegativity clauses of TV non-expansiveness (the doubled identity doubles the basis pair's TV `2 > 1`; the signed stochastic fixture `!![2,−1;−1,2]` triples it `3 > 1`), the row-sum clause of Dobrushin submultiplicativity (`δ(Qx²) = 2 > δ(Qx)² = 1` at `Qx = !![1,1;1,−1]`, `s = t = 1`), and the zero-mass clauses of both pairing cores (`1 ≤ 1/2` with every oscillation clause verified at `D = 1`) — each fence the negation of the conclusion at a specific instantiation with both sides pinned to numerals, each with an isolation companion proving the refuted clause is exactly what fails at the fixture (all other hypotheses verified genuine); recorded non-fenceable: `0 < δ` in the Doeblin contraction is a strength hypothesis (at `δ ≤ 0` the statement stays true by non-expansiveness), and the `[Nonempty V]` arguments are structural; score held at 4.5 per protocol — negative witnesses and isolation companions of an already-counted family, not a new QA capability family; the *randomized* half of the axis's named gap remains absent). Earlier sync: 3768 declarations across 68 modules (count synced 2026-09-02 with the primitivity-supplier QA +20, 3748 → 3768 — `EmpiricalStationary_QA.lean`'s Primitivity section, `proposals/primitivity-supplier-plain-walk.md`: the supplier's `IsPrimitive` output instantiated on the triangle through the explicit odd walk `0→1→2→0`, the primitive square pinned (`P² = (1/4)(J−I)` with the `m = 1` zero-diagonal fence), **the Doeblin rate attained exactly at the even time `t = 2`** (truth `TV = 1/6` = the engine's `(1/4)^{2/2}·(2/3)` — the start factor exactly right, not merely an upper bound) with the odd-time factor-`2` slack pinned beside it, the `K₂` periodicity fence (`(P^t) 0 0 = 0` at every odd `t` — the `hodd` hypothesis exactly isolating the bipartite class the lazy program patches), the self-contained capstone instantiated at `ε = 1/4` with the produced threshold's arithmetic (`⌈log 8/log 4⌉ = 2`) and its honest slack witnessed on the truth side (`TV(ν₂) = 1/6 > 1/8`), and the convergence corollary instantiated; score held at 4.5 per protocol — QA witnesses and exact/fence pins of a composition family, not a named-gap closure; the *randomized* half of the axis's named gap remains absent). Earlier sync: 3748 declarations across 68 modules (count synced 2026-09-02 with the parametric cycle QA +10, 3738 → 3748 — `AlonBoppana_QA.lean`'s `Step8` section, `proposals/parametric-cycle-qa.md`: **the repository's first QA section whose lemmas quantify over the scale** — every pin `∀`-quantified over `n`/`k`/`m` and consuming the shelf theorems at symbolic scale, so a scale-dependent error in a parametric statement fails at *every* scale at once where a fixed fixture (largest ever: `C₁₂`) can only fail below its own size: `dist a (a+⟨m⟩) = m` with the antipodal tie *in* the hypothesis set, the translate-invariant edge pin, the family's own antipode at the exact tie, the exact `distEdge = 2k+3` (strictness slack exactly `1` at every scale), **the tree-ball truth boundary bracketed at every scale** — the even boundary class computed raw from `cycle_levE_eq` exactly where `cycle_levClass_eq`'s strict hypothesis refuses (the class is the adjacent antipodal pair, card `2`, correcting the cycle-family delivery record's boundary note), `IsTreeBall` holding at radius `n/2` one past the shelf theorem's certified radius (strictness proof-forced, not truth-forced), failing one past the truth with the empty class, the odd-boundary singleton (card `1`, the genuine strictness bite), and the asymptotic corollary's witness pinned explicitly at symbolic `ε` (`k = ⌈1/ε⌉`). **Re-scored 4.0 → 4.5 per the "named absent category gains its first occupant" protocol: the axis's named gap "parametric property QA" — cited by every score-hold since 2026-08-22, with the cycle-family delivery's own record conceding its pins "remain fixed-`n`" — gains its first occupants, a new QA capability family, not packaging. The *randomized* half of the named gap ("parametric/randomized QA") remains absent and honestly recorded: no randomization machinery for QA exists in the repository.**) Earlier sync: 3738 declarations across 68 modules (count synced 2026-09-02 with the self-contained PageRank capstone QA +4, 3734 → 3738 — `EmpiricalStationary_QA.lean`'s `PageRankSelfContained` section, `proposals/selfcontained-empirical-pagerank.md`: **the identification pin** — any vector carrying the self-contained theorem's three certificate clauses equals the hand-verified `u2`, through the proved `∃!`'s uniqueness clause (the composition's falsification surface, load-bearing on the re-proved stationary layer's exact statement) — the display-threshold pin (`⌈log 8/log 2⌉ = 3` at `ε = 1/4`), the display-slack witness (the exact uniform object `2 < 3` = the display threshold), and the fully-self-contained instance (vector identified, guarantee discharged at `t₀ = 3` for every start, bound `2 exp(−1/32)`); score held at 4.0 per protocol — the QA witnesses of a composition theorem, not a new theorem family in QA terms). Earlier sync: 3734 declarations across 68 modules (count synced 2026-09-02 with the Cesàro stationary-existence QA +22, 3712 → 3734 — `IrreducibleStationary_QA.lean`'s Section E, `proposals/cesaro-stationary-existence.md`: **the orbit pins `g₁ = (2, 1/2, 1/2)` and `g₂ = 1` with the oscillation fence `g₁ ≠ g₂` beside them** (the engine's input sequence never converges — only its averages do), **the one-period Cesàro mean exactly stationary** (`μ₁ ᵥ* P = μ₁` with the value pin `μ₁ = 3•π₃` — the averaging route's whole point on the periodic fixture where powers oscillate forever), power positivity instantiated at the pinned return arc `(P²) 1 1 = 1/2` and in existence form, **the engine's output identified** (every nonnegative mass-3 stationary vector = `3•π₃`), **the min-ratio scalar pinned exactly** (`c = 1/4` at `σ = 3•π₃`, `τ = 12•π₃`), and **the reducible-input existence instantiation** (the honest scope pin: existence holds where uniqueness and positivity provably fail — the existing fences); score held at 4.0 per protocol — mechanism witnesses and exact pins of a re-proved layer, not a new theorem family in QA terms). Earlier sync: | QA | 4.0 | 3712 declarations across 68 modules (count synced 2026-09-02 with the directed uniform (worst-start) `t_mix` QA +29, 3683 → 3712 — `DirectedMixing_QA.lean`'s Section H plus `EmpiricalStationary_QA.lean`'s uniform capstone instance, `proposals/directed-uniform-mixing-time.md`: **the two-start closed form `d(t) = (1/2)^t` with submultiplicativity attained with equality at every time**, **the new matrix-level Dobrushin contraction attained exactly** at the basis pair (both sides `1/2` — the engine's sharp constant load-bearing on the directed axis), `d̄ = d/2` pinned beside the domination instance, the mixed form attained at every time, **the uniform object `t_mix^unif(1/8) = 2` pinned in both directions** through the sup interchange, **the refined α-ceiling attained exactly** with the display form's slack witnessed (`2 < 3`), **the escalation corollary attained exactly** (`t_mix(1/32) = 4 = (1+1)·2` at the fixture's own pinned certificate constants), and the `ε = 0` junk corner fenced; score held at 4.0 per protocol — the field-standard packaging of the already-counted directed rate family into its worst-start object plus the matrix-level engine's exact pins, not a new theorem family in QA terms). Earlier sync: the directed `t_mix` object QA +15, 3668 → 3683 — `DirectedMixing_QA.lean`'s Section G plus `EmpiricalStationary_QA.lean`'s PageRank capstone section, `proposals/directed-mixing-time-object.md`: the object `t_mix(1/8) = 2` **pinned in both directions** on the periodic 2-cycle's Google matrix, **the α-ceiling attained exactly** (`⌈log 4/log 2⌉ = ⌈2⌉ = 2` = the object), **the depth form attained with equality at the threshold time** (`TV_2 = 1/8`), the new equal-mass entrywise TV extraction `|μ i − ν i| ≤ TV` attained with both sides pinned `1/4` (the constant sharp — a factor-`2` statement would read `1/4 ≤ 1/2`), the junk corner fenced at `ε = 0`, and the bias/capstone instances at `2 exp(−1/8)` / `2 exp(−1/32)` past the pinned threshold; score held at 4.0 per protocol — the field-standard packaging of the already-counted directed rate family into its `t_mix` object plus the sampling capstone's twins and exact/fence pins, not a new theorem family in QA terms). Earlier sync: the Doeblin TV contraction and PageRank rate QA +31, 3637 → 3668, `DirectedMixing_QA.lean`'s Section F, `proposals/doeblintv-tv-contraction-pagerank-rate.md`: **the TV contraction attained exactly** at the basis pair (`TV = 1/2 = (1 − 2·(1/4))·1` — load-bearing on the exact coefficient, now on a second mathematical surface: TV between laws, not coordinate convergence — the Doeblin engine's second consumer), **the iterated contraction attained at every time** through the mode closed form, the **floor-free refutation fence** on the permutation fixture (the entries-floor hypothesis exactly what fails), and **the PageRank `α^t` rate attained exactly at every time on the periodic 2-cycle** — the periodic chain that provably never mixes (`P2_no_limit`) becomes exactly-`α`-convergent after teleportation, with the plain walk's never-decay `TV ≡ 1/2` pin beside it; score held at 4.0 per protocol — a new consumer plus mechanism witnesses and exact/fence pins of an already-counted family, not a new theorem family in QA terms). Earlier sync: the primitive-power retirement's mechanism QA +19, 3618 → 3637, `DirectedMixing_QA.lean`'s Section E, `proposals/retire-primitive-power-convergence.md`: **the Doeblin contraction attained exactly** on the strictly positive `2×2` fixture (`1 = (1 − 2·(1/4))·2` — the strongest QA shape a bound theorem can have, load-bearing on the exact coefficient), the iterated block contraction attained at every time through the exact closed form `Qd^t *ᵥ yd = (1/2)^t • yd`, the retired theorem's instance joined to that raw closed form, and the **wrong-δ refutation fence** (a pretend `δ = 1/2` claims zero range after one step — refuted at pinned values with the entries-bound hypothesis exactly what fails); score held at 4.0 per protocol — the retirement's mechanism witnesses plus exact/fence pins, not a new theorem family in QA terms. Earlier sync: the cycle-family +6 (3612 → 3618, `AlonBoppana_QA.lean`'s Step-7 section, `proposals/cycle-family-alon-boppana-asymptotic.md`: the exact cycle distance formula pinned at `C₁₂` in four shapes — short, wraparound, antipodal tie, self — the level class `{4, 9}`, the theorem instance at `k = 1`, the Laplacian spelling, the asymptotic instance at `ε = 1/2`, and the far-apart fence (`C₁₂` at `k = 2`: both radius-3 tree balls genuine, `hfar` failing by exactly the family-sizing slack) — the *theorems* being the program's first parametric (arbitrary-`n`) instantiation, though these QA witnesses themselves remain fixed-`n`, so the axis's full parametric-QA gap is narrowed, not closed; score held at 4.0 per protocol). Earlier sync: the empirical-lazy-sampling +12 (3600 → 3612 — `EmpiricalStationary_QA.lean`'s lazy section, `proposals/empirical-lazy-stationary-sampling.md`: the **plain-certificate unsatisfiability fence** on the bipartite path (no `r ∈ (0,1)` rate certificate for the plain walk exists there — the delivery's leverage case made negative), the bias-term instance at the computed intrinsic-rate bias `(1/2)²·√3/2 = √3/8` with the raw true deviation `1/8` dominated with honest slack beside it, the depth-form capstone instance closing at `2 exp(−1/16)` past the same `log(4√3)/log 2 ≤ 3` threshold the lazy-ceiling QA pinned, the event-mass non-vacuity witness `25/256`, and the exact-stationary contrast (true deviation `0` at every `t₀ ≥ 1` against the honestly positive quoted bias) — the lazy twins of already-counted sampling families instantiated on the fixture class the plain program cannot reach, plus exact/fence pins; score held at 4.0 per protocol). Earlier syncs: the lazy mixing-time +11 (3589 → 3600 — `Mixing_QA.lean`'s lazy-mixing-time section, `proposals/lazy-mixing-time-objects.md`: the exact corner-start TV closed form `TV_lazy(1+t) = (1/2)^{t+2}` at every time on the bipartite path, **`t_mix_lazy(corner, 1/8) = 2` pinned in both directions** where the plain walk provably never mixes, `t_mix_lazy(center, 1/4) = 1` with the ceiling attained exactly, the corner ceiling with honest slack (`3` vs `2`), **the object-level periodicity contrast** (plain `t_mix(K₂, 1/8) = 0` junk — empty witness set — against lazy `= 1` genuine), and the entrywise bound attained exactly on `K₂` (both sides zero at rate `0`) — the field-standard packaging of the already-counted lazy and `t_mix` families plus exact/fence pins; score held at 4.0 per protocol). Earlier syncs this day: the lazy-walk +23 (3566 → 3589 — `Mixing_QA.lean`'s LazyWalk section, `proposals/lazy-walk-mixing.md`: on `K₂` the intrinsic-rate bound **attained exactly at every time** (`χ²_lazy ≡ 0` at rate `1 − 2/2 = 0`), the TV periodicity-fix contrast pair (plain `1/2` vs lazy `0`), on `P₃` the periodic center start exactly stationary after one lazy step against the proved plain never-decay pin `χ²_plain ≡ 1`, the corner values with the bound instance at the pinned gap, the signless-engine tightness pin at the bipartite top mode, and the `hnn` fence at a negative-diagonal fixture — a time-change of the already-counted mixing families plus exact/fence pins; score held at 4.0 per protocol). Earlier syncs this day: the uniform mixing-time +27 (3517 → 3544), the spectral floor +30 (3487 → 3517), the discrete `t_mix` +9 (3478 → 3487), the Poisson bridge +19 (3459 → 3478). (Prior text: 3544 declarations, count synced 2026-09-01 with the uniform mixing-time delivery's QA +27, 3517 → 3544 — `Mixing_QA.lean`'s `UniformMixing` section, `proposals/total-variation-mixing-conversion.md`'s deferred uniform-`t_mix` family delivered with the submultiplicativity class: the distance pins `d(1) = 1/2`, `d(2) = 1/4`, `d̄(1) = 1/3`, `d̄(2) = 1/6` on the triangle from raw law literals, **submultiplicativity attained with equality** at `(1,1)` in all three forms (the sharp Dobrushin constant load-bearing — a factor-`2` statement would read `1/4 ≤ 1/2`), the uniform object's exact closed forms `t_mix(1/3) = 1`, `t_mix(1/6) = 2` pinned in both directions, the sup-interchange instance, the uniform ceiling instance, and the `K₂` periodicity corner at the uniform level (`d(1) = 1` maximal, no uniform witness, the empty-set infimum pinned and fenced) — exact pins and fences within the already-counted fixture families, the field-standard packaging of the already-counted TV families, no new parametric-property category opened; score held at 4.0 per protocol).
...
its remaining gap was parametric property QA and eigenvalue pinning
beyond two-point fixtures; the eigenvalue half closed with the
C₄–C₁₂ spectral pins, and the parametric half closed 2026-09-02
(`proposals/parametric-cycle-qa.md`: the cycle family's theorems
exercised at symbolic scale, every QA lemma `∀`-quantified over the
scale — the axis re-scored 4.0 → 4.5 per the "named absent category
gains its first occupant" protocol; the randomized half of the named
gap remains absent, honestly recorded). Mathlib interoperability was
re-scored 3.5 → 4.0 on 2026-08-18 (`supportGraph`: the first
