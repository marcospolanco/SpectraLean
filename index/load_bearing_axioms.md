...
1. **HIGH**: matrix_bernstein, matrix_hoeffding, matrix_azuma_hoeffding, perron_frobenius (the four remaining admitted axioms — `primitive_power_tendsto` retired 2026-09-02 by the Doeblin/Dobrushin contraction route, `proposals/retire-primitive-power-convergence.md` — the matrix trio has theorem consumers; the scalar family is fully proved: `subgaussian_tail_bound` retired 2026-08-22, `hoeffding_inequality`/`hoeffding_empirical` and `bernstein_inequality`/`bernstein_bounded_variance` and `hoeffding_lemma` retired 2026-08-30). **`perron_frobenius` state change, 2026-09-02** (`proposals/cesaro-stationary-existence.md`): its entire theorem-consumer surface — `IrreducibleStationary`'s engine plus four stationary theorems, `PageRank`'s three, and every `DirectedMixing`/empirical instantiation producing `π` through them — was re-proved without the axiom by the Cesàro/power-positivity/min-ratio route (`#print axioms` exactly the standard three across 37 audited declarations), so the axiom now has **zero non-QA consumers**. It remains admitted at its own full statement (rootMultiplicity + complex-domination clauses untouched, its own QA including the `-- @refutes` imprimitivity fence still exercising it); whether to open the §9 deprecation window is an operator decision, not a default.
