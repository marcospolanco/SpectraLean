...
| `Quantization.quantize` / `Quantization.quantize_mul_step` / `Quantization.abs_sub_quantize_le` | Numerical robustness (2026-09-07) | **The uniform `b`-bit quantizer over a known range** `(R/2^b) · round(x · 2^b/R)`, its bucket-midpoint fixed points (the closed-range `2^b + 1`-distinct-values substance), and the half-bucket error bound `\|x − quantize\| ≤ R/2^(b+1)` — pure rescaling of Mathlib's `abs_sub_round`; the repository's first quantization apparatus (`proposals/quantized-cheeger-bound.md`) |  |
| `secondEval_regularNormalizedLaplacian_nonneg` / `_le_two` | theorem (2026-09-07, range bridge) | The proved a priori spectral interval `0 ≤ λ₂(L_sym) ≤ 2` of the regular normalized Laplacian — the cap transported from `secondEval_normalizedLaplacian_le_two` through the `d`-regularity agreement `normalizedLaplacian_eq_regularNormalizedLaplacian` + `secondEval_congr`, the floor read off `evals` at the sorted second entry through PSD-ness; the range the quantizer's `R = 2` choice rests on |  |
| `quantized_cheeger_le` | theorem (2026-09-07, numerical robustness) | **The quantized Cheeger inequality**: `(λ̃₂ − 1/2^b)/2 ≤ φ(G) ≤ √(2(λ̃₂ + 1/2^b))` with `λ̃₂` the `b`-bit quantized spectral gap — pure algebra over the two proved Cheeger theorems plus the quantizer's error bound, the error entering linearly below and inside the square root above; hypothesis set exactly the Cheeger pair's |  |
...
| `quadForm_laplacian_partIndicator` | **the per-part energy identity**: `quadForm (laplacian A) (partIndicator S) = boundary A S` — the k = 2 family's `cutTestVector` energy identity at the indicator level, for arbitrary part counts; hypothesis-free (ANY weights, asymmetric included — the delivery's provisional `hA` clause found truth-removable by the 2026-09-07 corner audit and removed, the outflow route never consuming symmetry; the delivered `…_unsymm` twin retired with it) |
