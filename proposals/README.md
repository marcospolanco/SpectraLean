...
Note: with Courant–Fischer delivered, nothing is currently marked High;
the next run should either pick a Medium item above (Sherman–Morrison is
the cheapest retirement; Fiedler Phase B needs its named operator
decision) or continue the center-out queue in `docs/EXECUTION_PLAN.md`
(candidate follow-ons unlocked by the min–max theorem: the interlacing
retirement — the proposal's first named consumer — and full
Rayleigh/Dirichlet monotonicity, both needing separate scoping runs).
