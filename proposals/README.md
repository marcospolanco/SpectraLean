...
| --- | --- | --- |
| High | [Repair and Retire the Woodbury Identity Axiom](repair-and-retire-woodbury.md) | Reconsidered 2026-08-18: not a leverage bet, a correctness repair — the current admitted axiom is independently verified *false* (counterexample: `A=1, U=V=1, C=0`), and Mathlib already has the correct proof (`Matrix.invOf_add_mul_mul`). Zero current consumers, but `GraphTheory/Dynamics.lean` names this identity as its own intended future one; repairing it before that wiring happens is cheaper than discovering the break afterward. |
