...
| Metric | Count |
| --- | ---: |
| QA theorem/lemma declarations | 96 |
...
| Direct QA module targets | Pass | 2026-08-17 | All fifteen QA modules compiled individually (`lake build Scaffold.QA.…`) with no `sorry`/`admit`, including `SpectralGraph/Normalized_QA.lean`. |
...
- **QA declaration:** a theorem or lemma under `Scaffold/QA`; all 96 declarations have compiled against the current public API.
