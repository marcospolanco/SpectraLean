...
| QA theorems/lemmas | 2520, with no `sorry` or `admit` under `Scaffold/` |
...
`GraphTheory.AlonBoppana`, Step 1 of the operator-adopted
...
the lower companion to the delivered Cheeger/expander toolkit, with
the tree-ball test-vector steps priced by the Step-0 spike and gated
on the proposal's one-step-per-run instruction); the scalar and matrix
