...
| QA theorems/lemmas | 3734, with no `sorry` or `admit` under `Scaffold/` |
...
| Nonnegative-matrix spectral theory | `LinearAlgebra.PerronFrobenius` (the admitted Perron–Frobenius axiom) and `LinearAlgebra.PrimitiveConvergence` — the primitive power limit **proved 2026-09-02** by the Doeblin/Dobrushin contraction route (retired from axiom; the entrywise-range engine `entryRange_mulVec_le_of_pos_entries` is public and reusable) |
