...
| Metric | Count |
| --- | ---: |
| QA theorem/lemma declarations | 307 |
...
| Direct QA module targets | Pass | 2026-08-18 | All twenty-two QA modules compiled individually (`lake build Scaffold.QA.…` / direct elaboration) with no `sorry`/`admit`, including the repaired `SpectralGraph/Cheeger_QA.lean` (axiom-shape refutation + eigenvalue value-pinning), `SpectralGraph/Connectivity_QA.lean` (connected positive witness + disconnected negative witness for the kernel characterization), `SpectralGraph/SimpleGraphAdapter_QA.lean` (adapter agreement + kernel-span failure on a disconnected witness), and the new `SpectralGraph/KernelBridge_QA.lean` (kernel-equality bridge coherence + disconnected component count/basis computed independently of the transferred theorem). |
| Direct public module targets | Pass | 2026-08-18 | All public modules compile individually; the modules changed in the 2026-08-18 bridge slice (`GraphTheory.Spectral` extended, `GraphTheory.SimpleGraphAdapter` extended) were re-elaborated and rebuilt directly (`lake env lean` zero errors/zero new warnings; `lake build` of the adapter target) alongside the umbrella build. |
...
- **QA declaration:** a theorem or lemma under `Scaffold/QA`; all 307 declarations have compiled against the current public API.
