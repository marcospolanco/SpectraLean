...
| QA | 4.5 | 5494 declarations across 68 modules (count synced 2026-09-05 with the stationary family fence audit +72, 5422 → 5494 — the `AdversarialFences` section of `Stationary_QA.lean`, `proposals/adversarial-fences-stationary-family.md`, this run's own fresh reverse-import walk confirming Stationary as the top unaudited shelf at 4 transitive non-QA consumers (`Mixing`, `Oversmoothing`, `DirectedMixing`, the `EmpiricalStationary` capstone), its 259-line QA pre-discipline with one free-form negative witness: thirteen hypothesis-form fences — the six `hA` clauses at the delivered asymmetric `asymAdj2` where the kill mechanism is that `Pᵀ *ᵥ deg` computes *column* sums against the row-sum degree vector (the degree-measure fence consuming the delivered free-form witness, reconciling it into the discipline), the degree-measure `hd` clauses at the new signed canceling-zero-degree triangle `!![0,1,−1;1,0,0;−1,0,0]]` where the junk `√0 = √(−1) = 0` collapses the congruence and the junk `0⁻¹ = 0` zeroes the zero-degree row's column contribution (nonnegative zero-degree rows are inert — signs are what make them load-bearing), the stationary-measure and symmetrized `hd` twins at a volume-nonzero variant fixture (the first triangle's vol `0` would junk-collapse both sides of the division), and the mass-conservation `hd`/`hdpos` pair at the wrong claimed degree and the `d = 0` junk corner — plus five proved strengthening companions recording the structural findings: the kernel statement is P4 truth-removable in its `hA` (the shelf proof never consumes symmetry, and the symmetry-free companion makes that machine-checked) and the positivity clauses are secretly nonzeroness clauses (`deg ≠ 0` suffices by the shelf proofs' own routes; the regular one transfers the random-walk audit's `d = 0`-corner finding to its consumer); all 75 declarations at `propext, Classical.choice, Quot.sound`, zero axiom contact (all-proved shelf, theorem-instantiation fences, no `-- @refutes` tags; 12-tag independence check unchanged and clean); score held 4.5 per protocol — negative witnesses of an already-counted family, not a named-gap closure; previous sync +27, 5395 → 5422 with the random-walk family audit — six fences plus the `hdpos`-only-through-`d = 0` exact characterization; previous sync +88, 5307 → 5395 with the heat family audit — 30 fences over the exponential engines, rank-one collapse, decay factors, DC limit, variance-decay twins, conservation clauses, and eigenvalue plumbing at five fixtures; previous sync +33, 5274 → 5307 with the Perron–Frobenius family audit — three tagged axiom-independent fences closing the admitted axiom's full three-clause hypothesis surface, tag count 9 → 12). |
