...
| QA | 4.5 | 5395 declarations across 68 modules (count synced 2026-09-05 with the heat family fence audit +88, 5307 → 5395 — the `HeatFences` section of `Heat_QA.lean`, `proposals/adversarial-fences-heat-family.md`, this run's own fresh consumption survey confirming Heat as the library's most-consumed unaudited shelf at 8 transitive non-QA consumers: 30 hypothesis-form fences at four new fixtures — the negative edge `!![0,-2;-2,0]]` whose Laplacian carries eigenvalue `-4` (dissipation failure, DC-limit failure, variance-decay `hnn` failure `2e⁸ > 2`, below-gap `hnn` `0 ≤ -4`, both π-isometry `hd` junk-√ kills), the zero-degree signed `!![-2,2;2,0]]` (six normalized/walk `hd` kills through the junk `D⁻¹ᐟ²` congruence-collapse with `√D·1` surviving), the signed 2-regular `!![3,-1;-1,3]]` (normalized `hnn` `0 ≤ -1` and walk-variance `hnn` `4e² > 4`), and the asymmetric rank-one `!![0,2;1,0]]` (mean-preservation `hA` kill `1 − α ≠ 1`), plus the engine-level `hM`/`hc`/`hμ` fences at K₂, the nilpotent, the identity, and the delivered `disAdj`; all 93 declarations at `propext, Classical.choice, Quot.sound`, zero axiom contact; score held 4.5 per protocol — negative witnesses of an already-counted family, not a named-gap closure; previous sync +33, 5274 → 5307 with the Perron–Frobenius family audit — the `AdversarialFences` section of `PerronFrobenius_QA.lean`, `proposals/adversarial-fences-perron-frobenius-family.md`, the prior handoff's named top target confirmed at 7 transitive non-QA consumers: three tagged axiom-independent hypothesis-form fences closing the admitted axiom's full three-clause hypothesis surface — `hnn` at the negative-diagonal irreducible `!![-1,2;2,-1]]` where the eigen conjuncts stay satisfiable in isolation (`r = 1`, `x = (1,1)` pinned) so the kill route is genuinely the domination clause (`3 ≤ 1`), `hirr` at the reducible nilpotent edge killed through the eigen clause alone, and `hex` at the one-vertex zero matrix, the module documentation's own named corner now a proved fence — plus the `IsIrreducible` positive-arcs definitional witness; all 38 declarations at `propext, Classical.choice, Quot.sound`, zero axiom contact, repository tag count 9 → 12 all mechanically independence-checked; score held 4.5 per protocol — negative witnesses of an already-counted family, not a named-gap closure; previous sync +34 with the resolvent family audit, `Resolvent_QA.lean`'s 12 fences reconciling its five free-form witnesses, headline findings the `Fin 0` corner as the *only* `hc` failure corner of both upper norm bridges, the one-sided singular-shift separation of the resolvent identity's determinant clauses, and the `4×` Lipschitz PSD separation `3 ≤ 3/4`). |
