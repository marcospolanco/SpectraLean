...
and the full weighted component structure are proved, but no electrical
quantity is defined yet) and 8 (adjacent systems — deliberately
...
structure is now complete; the score stays below 2.0 because no
electrical quantity — resistance, Matrix–Tree, Kirchhoff — is defined
yet).
