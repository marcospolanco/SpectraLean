...
| QA | 4.0 | 3478 declarations across 68 modules (count synced 2026-08-31 with the continuous-time mixing-time delivery's QA +26, 3433 → 3459 — `Mixing_QA.lean`'s `ContMixingTime` section, `proposals/continuous-time-chi-square-mixing.md`'s Deferred items 1+2: the exact `K₂` mixing-time closed form `t_mix(ε) = ln(1/(2ε))/2` (a `sInf` pinned in both directions), the ceiling attained exactly at `ε = e^{−2}/2`, the exact TV values on `K₂` (attained at every time) and the triangle `(2/3)e^{−3t/2}` with the Cauchy–Schwarz slack proved strict, the wrong-gap refutation, the big-`ε` corner, and the antitone instance with closed-form consistency; score held at 4.0 per protocol — a consumer composition of the already-counted TV and continuous-time χ² families plus exact/fence pins, no new parametric-property category opened). Earlier syncs this day: the continuous-time χ² delivery +19 (3401 → 3420) and its path-fixture deferred follow-on +13 (3420 → 3433). **Further synced 2026-09-01 to 3478** (+19) after the same proposal's Poisson-bridge follow-on (continuous↔discrete mixing comparability, `Mixing.lean`'s Poisson-bridge section) landed without its own terminal delivery record; count independently re-verified by the commit steward (`#print axioms` on all new declarations, `check_build_completeness.py`, full ladder), score held at 4.0 pending that run's own fuller radar narrative.
