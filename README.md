...
machinery, `1 < d` load-bearing and fenced at K₂; and the **energy
half** followed the same day as Step 3's second sub-slice — the
level-Lipschitz lemma and the parent lemma `exists_levE_parent`
(every level-`≥ 1` vertex has a neighbor exactly one level down, no
connectivity hypothesis), the interior `interiorE` with its
level-sum bridge, the numerator bound **`radialVec_quadForm_ge`**
`2 + 4 k (d−1) ρ ≤ xᵀAx` harvested from below against the existing
`IsTreeBall` (one parent edge per interior vertex, both orders, plus
the endpoint edge — the C₈ QA pin tight at equality, with the
pseudo-edge, fractional-weight, and loop fences isolating the three
new hypotheses), and the Rayleigh corollary
**`radialVec_rayleigh_ge`** `(2 + 4 k (d−1) ρ) / (2 (k+1)) ≤ R` —
Nilli's quotient before the Step-5 `√` packaging; the **two-vector
orthogonalization** followed the same day as Step 4 — the equal-mass
lemma (both far-apart edges' radial vectors carry the same total mass
under `IsTreeBall`), the two-edge vector `twoEdgeVec` orthogonal to
`onesVec` at squared norm exactly `4 (k+1)`, the cross-edge
elimination through 3b's own level-Lipschitz lemma at the exact
far-apart threshold `2(k+1) < distEdge`, and the headline
**`twoEdgeVec_secondEval_le`** `secondEval (d•1 − A) ≤ d − (2 +
4k(d−1)ρ)/(2(k+1))` through `secondEval_le_rayleigh` — the program's
first eigenvalue-level statement; and the **program completed
2026-08-27 with Step 5, the diameter-dependent single-graph
statement**: the `√` packaging at `ρ = √((d−1)⁻¹)` delivering the
capstone **`alonBoppana_nilli`** — `secondEval (d•1 − A) ≤ d −
(1 + 2k√(d−1))/(k+1)`, Alon–Boppana's `2√(d−1)` barrier with the
error explicit — the classical error shape
`alonBoppana_nilli_classical` (`≤ d − 2√(d−1) + 2√(d−1)/(k+1)`), the
far-apart-to-diameter bridge `distEdge_le_diam` (the Step-2 priced
`ediam ≠ ⊤` blocker dissolved via finiteness-attainment +
connectivity), and the honest hypothesis-side diameter bookkeeping
`alonBoppana_diam_ge` (`k+1 ≤ ⌊diam/2⌋` — the tree-ball hypothesis
never derived from the diameter, per the proposal's own
qualification-trap warning), with both priced QA residuals delivered:
the loop-pair fence isolating `IsTreeBall` at a loop's one-element
level 0, and the independent integer-witness engine route reaching
`secondEval (2•1 − C₈) ≤ 2/3` — strictly stronger than the theorem
routes' `≤ 1` — at `![1,1,0,−1,−1,−1,0,1]` with fully raw arithmetic
— all proved hard crust, zero new axioms, the whole program
conditional on nothing); the scalar and matrix
...
| Alon–Boppana program | `GraphTheory.AlonBoppana` (the adopted program's home module, **Steps 1–5 complete — program COMPLETE 2026-08-27**, the d-regularity interface in the shelf's `d : ℝ` hypothesis idiom — `IsDRegular A d`, the constant-eigenvector fact `A *ᵥ onesVec = d • onesVec` (a row-sum computation), the **AM–GM row-sum domination** `xᵀAx ≤ d · ‖x‖²` (entrywise AM–GM multiplied through nonnegative weights; the symmetric double sum's halves both `d‖x‖²` via the row- and column-sum degree identities), and the **top-eigenvalue identification** `evals ⟨last⟩ = d` proved from both sides (the `onesVec` eigenvalue witness + domination at the unit eigenvector — load-bearing on the sorted-spectrum API at its extremes); Step 2's tree-ball interface at module level — `levE`/`levClass`/`ballE` (BFS levels of an edge against `SimpleGraph.dist` on `supportGraph`), the junk-zero-honest level-0 iff with its connected amortization, the **`IsTreeBall`** full-level predicate (`#{z | levE z = j} = 2 (d−1)^j`), the ball algebra with the **layer-cake cardinality bridge `ballE_card_eq_sum`**, and the far-apart condition `distEdge` + `ballE_disjoint_of_lt_distEdge`; Step 3's first sub-slice — `radialVec` (Nilli's radial test vector `ρ^{lev}` on the radius-`k` edge ball, the d-regular-tree normalization `ρ² = ((d−1:ℕ):ℝ)⁻¹` carried as a hypothesis) with the support/entry/nonvanishing interface, the **layer-cake sum bridge `sum_ballE_eq_sum_levels`**, and the **squared-norm identity `⟨ρ^{lev}, ρ^{lev}⟩ = 2 (k+1)`** exactly under `IsTreeBall` (the geometric growth of full levels cancelling the vector's geometric decay per level — the Rayleigh quotient's denominator, `1 < d` load-bearing at the cancellation and fenced at K₂); and Step 3's second sub-slice — the energy half: the **level-Lipschitz lemma** `levE_le_levE_add_one_of_adj`, the **parent lemma** `exists_levE_parent` (every level-`≥ 1` vertex has a support-adjacent neighbor exactly one level down, no connectivity hypothesis — the from-below harvest dissolving the "level sizes, not edge counts" obstruction without strengthening `IsTreeBall`), the interior `interiorE` with its level-sum bridge, the headline **numerator bound `radialVec_quadForm_ge`** `2 + 4 k (d−1) ρ ≤ xᵀAx` under the existing tree-ball predicate plus a `0`-or-`≥ 1` weight discipline, distinct endpoints, and a genuine edge (QA pinning it tight at equality on C₈ and isolating each new hypothesis with its own fence), and the **Rayleigh corollary `radialVec_rayleigh_ge`** `(2 + 4 k (d−1) ρ) / (2 (k+1)) ≤ R` — Nilli's quotient before the Step-5 `√` packaging; and Step 4 — the **two-vector orthogonalization**: the equal-mass lemma `radialVec_sum_eq` (both balls' vector masses the same function of `(d, ρ, k)`, so `twoEdgeVec := radialVec(x,y) − radialVec(u,v)` is orthogonal to `onesVec`), the norm identity `4 (k+1)` on disjoint radius-`k` balls, the cross-edge elimination through the level-Lipschitz lemma at the exact far-apart threshold `(k+1)+(k+1) < distEdge`, and the headline **`twoEdgeVec_secondEval_le`** `secondEval (d•1 − A) ≤ d − (2 + 4k(d−1)ρ)/(2(k+1))` through `secondEval_le_rayleigh` at `M = d•1 − A` (PSD and kernel from the Step-1 facts) — the program's first eigenvalue-level statement, QA-pinned on C₈'s antipodal pair (orthogonality two routes, numerator `8 = 6 + 6 − 2·2` raw by decomposition, the `hfar` and overlap fences, the instance `secondEval (2•1 − abC8) ≤ 1`); and the program **completed 2026-08-27 with Step 5** — the `Packaging` section: the `√` plumbing `mul_sqrt_inv_eq_sqrt`, the far-apart-to-diameter bridge **`distEdge_le_diam`** (the Step-2 priced `ediam ≠ ⊤` blocker dissolved: `exists_edist_eq_ediam_of_finite` + connectivity's `edist_ne_top_iff_reachable`), the honest diameter bookkeeping **`alonBoppana_diam_ge`** (`2(k+1)+1 ≤ diam`, the hypothesis-side `k+1 ≤ ⌊diam/2⌋` reading — the tree-ball hypothesis never derived from the diameter), the capstone **`alonBoppana_nilli`** `secondEval (d•1 − A) ≤ d − (1 + 2k√(d−1))/(k+1)` at `ρ = √((d−1)⁻¹)`, and the classical error shape **`alonBoppana_nilli_classical`** `≤ d − 2√(d−1) + 2√(d−1)/(k+1)` — Alon–Boppana's `2√(d−1)` barrier, error explicit, pure hard crust, zero new axioms, the program conditional on nothing; QA +9 (2585 total): the C₈ capstone instance (`≤ 2 − 1 = 1`), the classical instance documenting the error term swallowing the content at `k = 0` (`≤ 2`), the diameter instance `3 ≤ diam (supportGraph C₈)`, the loop-pair fence (a loop's level-0 class is `{2}`, `IsTreeBall` fails and the orthogonality fails with it, `⟨![1,1,−1],1⟩ = 1 ≠ 0`), and the independent integer-witness engine route `secondEval (2•1 − abC8) ≤ 2/3` — strictly stronger than both theorem routes' `≤ 1`, at a globally supported vector with fully raw arithmetic; and the program's first theorem consumer **delivered 2026-08-27** — the `ExpansionCeiling` section: **`ramanujan_expansion_ceiling`**, `cheegerConstant A ≤ √(2 (1 − 2√(d−1)/d + 2√(d−1)/(d (k+1))))` under exactly the capstone's hypotheses — the Alon–Boppana obstruction composed with the proved Cheeger hard direction into the textbook ceiling on expansion quality (obstruction scope only, no tightness claim), through the operator identity `smul_one_sub_eq_smul_regularNormalizedLaplacian` (`d • 1 − A = d • L_sym`) and two new `Spectral.lean` engine pieces (**`secondEval_smul_of_pos`**, exact positive scaling by the variational route — the constraint set of `c • M` is the positive-scaled image, `sInf` commuting; **`secondEval_congr`**, the proof-irrelevance bridge across equal operator spellings); zero new axioms, the whole chain unconditional; QA +7 (2666 total): the K₂ scaling pins by two independent routes (`secondEval (3 • L) = 6` raw vs theorem-joined — a wrong constant breaks exactly one) with the negative-`c` fence (`0 ≠ -2`), and the C₈ ceiling QA — the instance evaluating to `√2` at `k = 0`, the fixture's actual `φ ≤ 1/4` by the exhibited half-set cut (non-vacuous with strict slack, never tight), and the two-`k` improvement pinned on the statement's own constants) |
