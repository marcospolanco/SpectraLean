...
Laplacian increments are strongly measurable on the underlying space and
form a matrix martingale difference sequence with uniform spectral-norm
bound `R` (in particular every realization is `R`-event-driven, by
`randomLaplacianIncrement_bounded_iff`), the cumulative Laplacian
perturbation after `m` steps obeys
...
Statement history: the `h_adapt` hypothesis (strong measurability w.r.t.
the generated past — content-free, since that σ-algebra is generated by
the increments themselves) was replaced by ambient `h_meas` in the
2026-08-29 `MatrixMDS` repair
(`proposals/audit-matrix-azuma-mds-measurability-hazard.md`, Errata §7):
without ambient measurability the `h_cond` integrals are junk zeros and
the statement is false in hypothesis form.
...
theorem eventStreamTail [Nonempty V] {Ω : Type*} {mΩ : MeasurableSpace Ω}
  ...
  have hazuma := matrix_azuma_hoeffding m
    (⟨R, randomLaplacianIncrement A, h_meas, h_cond, h_bound⟩ :
